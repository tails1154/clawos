...
if(LIBWINPTHREAD_LOCATION MATCHES "mingw32")
    ...
    set(LIBGCC_MSVCRT_LOCATION ${CMAKE_BINARY_DIR}/sdk/cmake/libgcc_msvcrt.a)
    file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/sdk/cmake)
    file(WRITE ${LIBGCC_MSVCRT_DEF} "LIBRARY msvcrt.dll\nEXPORTS\n    __msvcrt_assert\n    fseeko64\n    ftello64\n    fstat64\n    lseek64\n    mbrtowc\n    mbsrtowcs\n    strnlen\n    wcsnlen\n    wcrtomb\n    wctype\n")
    execute_process(
        COMMAND ${CMAKE_DLLTOOL} --def ${LIBGCC_MSVCRT_DEF} --kill-at --output-lib=${LIBGCC_MSVCRT_LOCATION} -t libgcc_msvcrt
        RESULT_VARIABLE LIBGCC_MSVCRT_RESULT
        OUTPUT_QUIET
        ERROR_QUIET)
    if(LIBGCC_MSVCRT_RESULT EQUAL 0 AND EXISTS "${LIBGCC_MSVCRT_LOCATION}")
        add_library(libgcc_msvcrt STATIC IMPORTED)
        set_target_properties(libgcc_msvcrt PROPERTIES IMPORTED_LOCATION ${LIBGCC_MSVCRT_LOCATION})
        target_link_libraries(libwinpthread INTERFACE libkernel32 msvcrtex libgcc_msvcrt)
        target_link_options(libwinpthread INTERFACE "-Wl,--allow-multiple-definition")
    else()
        target_link_libraries(libwinpthread INTERFACE libkernel32 msvcrtex)
    endif()
else()
