...
Language=English
{GUID Substitution}
During the translation of a global identifier (GUID) to a ClawOS security ID (SID), no administratively-defined GUID prefix was found.
...
{EXCEPTION}
Cannot Continue
ClawOS cannot continue from this exception.
.
...
Language=English
{Bad Image}
%hs is either not designed for ClawOS or it contains an error.
...
Language=English
The SAM database on a ClawOS Server is significantly out of synchronization with the copy on the Domain Controller. A complete synchronization is required.
.
...
Language=English
The NtCreateFile API failed. This error should never be returned to an application, it is a place holder for the ClawOS Lan Manager Redirector to use in its internal error mapping routines.
.
...
ClawOS was unable to save all the data for the file %hs. The data has been lost.
...
Language=English
{ClawOS Evaluation Notification}
Your ClawOS will NEVER expire!.
.
...
The relocation occurred because the DLL %hs occupied an address range reserved for ClawOS system DLLs. The vendor supplying the DLL should be contacted for a new DLL.
.
...
Language=English
The ClawOS I/O reparse tag passed for the NTFS reparse point is invalid.
.
...
Language=English
The ClawOS I/O reparse tag does not match the one present in the NTFS reparse point.
.
...
Your system is low on virtual memory. ClawOS is increasing the size of your virtual memory paging file.
...
Language=English
ClawOS doesn't have any Small Business Support editions, so this error never appears.
.
...
Language=English
This version of ClawOS is not compatible with the behavior version of directory forest, domain or domain controller.
.
...
Language=English
The error specified is not a valid ClawOS RPC error code.
.
...
Language=English
ClawOS was not able to process the application binding information.
