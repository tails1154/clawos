...
Language=English
ClawOS could not be started as configured.  A previous working configuration was used instead.
.
Language=Russian
ClawOS could not be started as configured.  A previous working configuration was used instead.
.
Language=French
ClawOS n'a pas pu démarrer tel que configuré. Une précédente configuration fonctionnelle a été utilisé à la place.
.
Language=Polish
Nie można uruchomić systemu ClawOS zgodnie z aktualną konfiguracją. Zamiast niej użyto poprzedniej działającej konfiguracji.
.
Language=Romanian
ClawOS nu a putut fi pornit după configurație. În locul ei, a fost folosită o configurație funcțională anterioară.
.
