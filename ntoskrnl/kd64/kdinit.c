...
KdpPrintBanner(VOID)
{
    ...
    DPRINT1("-----------------------------------------------------\n");
    DPRINT1("ClawOS " KERNEL_VERSION_STR " (Build " KERNEL_VERSION_BUILD_STR ") (Commit " KERNEL_VERSION_COMMIT_HASH ")\n");
    ...
    _In_opt_ PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    if (EnableKd && (NT_SUCCESS(KdDebuggerInitialize0(LoaderBlock))))
    {
        ...
        /* Display separator + ClawOS version at the start of the debug log */
        KdpPrintBanner();
