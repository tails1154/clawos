...
/*
 * ClawOS uses the same boot screen for all the products.
 *
 * Enable this define when ClawOS will have different SKUs
 ...
 * - PltRotBarStatus == 1, do palette fading-in (done elsewhere in ClawOS);
    ...
    _In_ BOOLEAN TextMode)
{
    ...
    else
    {
         ...
         * to show that ClawOS can be still alive when the bar does not
         * appear to progress.
         */
...
GetFamousQuote(VOID)
{
    static const PCH FamousLastWords[] =
    {
        ...
        "ClawOS is ready to be booted again ;-)",
