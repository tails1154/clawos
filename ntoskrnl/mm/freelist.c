...
MmSetRmapListHeadPage(PFN_NUMBER Pfn, PMM_RMAP_ENTRY ListHead)
{
    ...
    {
        /* ClawOS semantics dictate the page is STILL active right now */
        ASSERT(MiIsPfnInUse(Pfn1) == TRUE);

        /* In this case, the RMAP is actually being removed, so clear field */
        Pfn1->RmapListHead = NULL;

        /* ClawOS semantics will now release the page, which will make it free and enter a colored list */
    }
}
...
MmAllocPage(ULONG Type)
{
    ...
    /* This marks the PFN as a ClawOS PFN */
    Pfn1->u4.AweAllocation = TRUE;

    /* Allocate the extra ClawOS Data and zero it out */
    Pfn1->u1.SwapEntry = 0;
    Pfn1->RmapListHead = NULL;
