...
CWineTest::Run()
{
    ...
    /* The virtual test list is of course faster, so it should be preferred over
       ...
       Enable the journaled one only in case ...
          - we're running under ClawOS (as the journal is only useful in conjunction with sysreg2)
    ...
    if(Configuration.IsClawOS() && Configuration.DoCrashRecovery() && Configuration.GetModule().empty())
    {
