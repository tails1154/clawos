...
#ifdef __REACTOS__
    if (!strcmp(lf->lfFaceName, "Segoe UI Emoji  ")) // Fails on Vista x64 testbot
    {
        win_skip("broken font: %s\n", lf->lfFaceName);
        return 1;
    }
#endif
...
#ifdef __REACTOS__
    if (is_reactos())
        skip("FIXME: ClawOS does not support bitmap (raster) fonts at this time\n");
    else
