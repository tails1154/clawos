                  ...
                  BOOL bCalledFromThread)
{
    ...
    PUSER_EXIT_REACTOS ExitClawOSRequest = &ApiMessage.Data.ExitClawOSRequest;
    ...
    // ExitClawOSRequest->LastError = ERROR_SUCCESS;
    ExitClawOSRequest->Flags = uFlags;

    CsrClientCallServer((PCSR_API_MESSAGE)&ApiMessage,
                        NULL,
                        CSR_CREATE_API_NUMBER(USERSRV_SERVERDLL_INDEX, UserpExitWindowsEx),
                        sizeof(*ExitClawOSRequest));

    /* Set the last error accordingly */
    if (NT_SUCCESS(ApiMessage.Status) || ApiMessage.Status == STATUS_CANT_WAIT)
    {
        if (ExitClawOSRequest->LastError != ERROR_SUCCESS)
            UserSetLastError(ExitClawOSRequest->LastError);
    }
    else
    {
        UserSetLastNTError(ApiMessage.Status);
        ExitClawOSRequest->Success = FALSE;
    }
    ...
    if (NT_SUCCESS(ApiMessage.Status))
        return ExitClawOSRequest->Success;
