...
typedef struct
{
    ...
    HBITMAP   hBmpClose; /* ClawOS modification */
} MDICLIENTINFO;
...
#ifdef __REACTOS__
	PostMessageA( hwnd, WM_MDICALCCHILDSCROLL, 0, 0 ); //// ClawOS: Post not send!
#else
...
void WINAPI CalcChildScroll( HWND hwnd, INT scroll )
{
    ...
    *
    * Apparently (in ClawOS at least) the rcClient returned by GetWindowInfo
