...

{
    ...
    COMPOUND_PATH_ENTRY CompoundPathEntry = {{0}};/* ClawOS Change: GCC "missing braces around initializer" */
    ...
    CD_NAME RemainingName = {{0}};/* ClawOS Change: GCC "missing braces around initializer" */
    CD_NAME FinalName;
        ...
        if (TypeOfOpen == UserFileOpen) {
            ...
            if (FsRtlCurrentBatchOplock( CdGetFcbOplock(Fcb) )) {
                ...
                OplockStatus = FsRtlCheckOplock( CdGetFcbOplock(Fcb),
                                                 IrpContext->Irp,
                                                 IrpContext,
                                                 (PVOID)CdOplockComplete,   /* ClawOS Change: GCC "assignment from incompatible pointer type" */
                                                 (PVOID)CdPrePostIrp );   /* ClawOS Change: GCC "assignment from incompatible pointer type" */
