...
AtaCalculateTimings(
    ...
    _In_ ULONG ClockPeriodPs)
{
    ATATIM DataCycleTime, CmdCycleTime;
    ...
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    ...
    else if (ChanData->Regs.Data == UlongToPtr(PCIIDE_LEGACY_PRIMARY_COMMAND_BASE))
    {
        ConfigInfo->AtDiskPrimaryAddressClaimed = TRUE;
        ChanData->ChanInfo |= CHANNEL_FLAG_PRIMARY_ADDRESS_CLAIMED;
    }
    ...
    _In_ PATA_CONTROLLER Controller)
{
    ...
    /* Assign DMA resources */
    if (!(Controller->Flags & CTRL_FLAG_MANUAL_RES))
    {
        PUCHAR IoBase;

        /* Check if BAR4 is available */
        IoBase = AtaCtrlPciMapBar(Controller, PCIIDE_DMA_IO_BAR, PCIIDE_DMA_IO_RANGE_LENGTH);
        if (!IoBase)
        {
            if (Controller->AccessRange[PCIIDE_DMA_IO_BAR].Flags & RANGE_IS_VALID)
                ERR("Failed to map bus master registers");
            else
                INFO("No bus master registers");
            return FALSE;
        }
        ...
        for (i = 0; i < Controller->MaxChannels; ++i)
        {
            ...
            if (Controller->AccessRange[PCIIDE_DMA_IO_BAR].Flags & RANGE_IS_MAPPED)
                ChanData->ChanInfo |= CHANNEL_FLAG_MRES_DMA;
            ...
            if (Controller->Flags & CTRL_FLAG_DMA_INTERRUPT)
                ChanData->CheckInterrupt = PciIdeCheckInterrupt;
        }
    }
    ...
    for (i = 0; i < Controller->MaxChannels; ++i)
    {
        ...
        if (!(ChanData->TransferModeSupported & ~PIO_ALL))
            continue;

        ASSERT(ChanData->Regs.Dma != NULL);

        DmaStatus = ATA_READ(ChanData->Regs.Dma + PCIIDE_DMA_STATUS, ChanData, MRES_DMA);
        if (!PciIdeIsDmaStatusValid(DmaStatus))
        {
            WARN("CH %lu: %p DMA 0x%02X\n", ChanData->Channel, ChanData->Regs.Dma, DmaStatus);
    ...
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    ...
    CommandPortBase = AtaCtrlPciMapBar(Controller, BarIndex, PCIIDE_COMMAND_IO_RANGE_LENGTH);
    if (!CommandPortBase)
        return STATUS_INSUFFICIENT_RESOURCES;

    ControlPortBase = AtaCtrlPciMapBar(Controller, BarIndex + 1, PCIIDE_CONTROL_IO_RANGE_LENGTH);
    if (!ControlPortBase)
        return STATUS_INSUFFICIENT_RESOURCES;

    if (Controller->AccessRange[BarIndex].Flags & RANGE_IS_MAPPED)
        ChanData->ChanInfo |= CHANNEL_FLAG_MRES_TF;

    if (Controller->AccessRange[BarIndex + 1].Flags & RANGE_IS_MAPPED)
        ChanData->ChanInfo |= CHANNEL_FLAG_MRES_CTRL;
    ...
    _In_ PCM_RESOURCE_LIST ResourcesTranslated)
{
    ...
    for (i = 0; i < ResourcesTranslated->List[0].PartialResourceList.Count; ++i)
    {
        ...
        switch (Desc->Type)
        {
            ...
            case CmResourceTypeMemory:
            {
                ...
                else if (Desc->u.Port.Length == PCIIDE_LEGACY_CONTROL_IO_RANGE_LENGTH)
                {
                    if (!ControlPortDesc)
                        ControlPortDesc = Desc;
                }
    ...
    if (CommandPortDesc->Type == CmResourceTypeMemory)
        ChanData->ChanInfo |= CHANNEL_FLAG_MRES_TF;

    if (ControlPortDesc->Type == CmResourceTypeMemory)
        ChanData->ChanInfo |= CHANNEL_FLAG_MRES_CTRL;
    ...
    _In_ PCM_RESOURCE_LIST ResourcesTranslated)
{
    ...
    if (Controller->Flags & CTRL_FLAG_NATIVE_PCI)
    ...
    _In_ BOOLEAN Enable)
{
    ...
    {
        UCHAR Status = ATA_READ(ChanData->Regs.Dma + PCIIDE_DMA_STATUS, ChanData, MRES_DMA);

        ATA_WRITE(ChanData->Regs.Dma + PCIIDE_DMA_STATUS, Status, ChanData, MRES_DMA);
    }
}
    ...
    _Inout_ PATA_CONTROLLER Controller)
{
    ...
    switch (Controller->Pci.VendorID)
    {
        ...
        case PCI_VEN_SERVERWORKS:
            Status = SvwPataGetControllerProperties(Controller);
            if (Status == STATUS_NO_MATCH)
                Status = SvwSataGetControllerProperties(Controller);
            break;
    ...
    if (Status == STATUS_NO_MATCH)
    {
        ...
        /* Disable 32-bit I/O for unknown controllers */
        for (i = 0; i < Controller->MaxChannels; ++i)
        {
            PCHANNEL_DATA_PATA ChanData = Controller->Channels[i];
     ...
     * NOTE: NT architecture does not support switching only one IDE channel to native mode.
     */
    if ((Controller->Pci.BaseClass == PCI_CLASS_MASS_STORAGE_CTLR) &&
        (Controller->Pci.SubClass == PCI_SUBCLASS_MSC_IDE_CTLR))
    {
    ...
    if (!PciIdeControllerInitDma(Controller))
    {
        for (i = 0; i < Controller->MaxChannels; ++i)
        {
            PCHANNEL_DATA_PATA ChanData = Controller->Channels[i];
