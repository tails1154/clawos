    ...
    IN OUT PUSETUP_DATA pSetupData)
{
    ...
    if (_wcsicmp(Value, L"$ClawOS$") != 0)
    {
        DPRINT("Signature not $ClawOS$\n");
        INF_FreeData(Value);
        goto Quit;
    }
    ...
    _Out_ PUNICODE_STRING SourceRootDir)
{
     ...
     *
     *          \SystemRoot ---> \Device\Harddisk1\Partition1\ClawOS
     * and:     \Device\Harddisk1\Partition1 ---> \Device\HarddiskVolume2
     * etc.
     * and we wish to resolve \SystemRoot to: \Device\HarddiskVolume2\ClawOS
     *
     ...
     *   i.e. \ClawOS , but not the whole NT path.
     *
...
/**
 * @brief
 * Verify whether the given directory is suitable for ClawOS installation.
...
DoUpdate:
    ...
    if (!RepairUpdateFlag || ShouldRepairRegistry)
    {
        ...
        if (!Success)
            Success = SpInfFindFirstLine(pSetupData->SetupInf, L"HiveInfs.Install", NULL, &InfContext); // ClawOS-specific
