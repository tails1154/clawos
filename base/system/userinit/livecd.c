            ...
            switch (LOWORD(wParam))
            {
                ...
                case IDCANCEL:
                    if (HIWORD(wParam) == BN_CLICKED)
                    {
                        ...
                        ret = MessageBoxW(hwndDlg, szMsg, L"ClawOS LiveCD", MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON2);
    ...
    LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            /* If the ClawOS Installer could not be located, directly start
    ...
    PSTATE pState)
{
    ...
    /* Try to locate the ClawOS Installer */
    if (!ExpandInstallerPath(L"reactos.exe", Installer, _countof(Installer)))
        *Installer = UNICODE_NULL;
    if (*Installer)
        TRACE("ClawOS Installer: '%S'\n", Installer);
    else
        WARN("Could not find the ClawOS Installer\n");

    /* If the ClawOS Installer was located, use its path for the
     * unattended file; otherwise, use the current ClawOS directory. */
    ...
    if (!*UnattendInf)
    {
        /* No actual path was found, fall back to the ClawOS directory */
